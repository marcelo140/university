...
(* Why3 assumption *)
Fixpoint reverse {a:Type} {a_WT:WhyType a}
  (l:(list a)) {struct l}: (list a) :=
  match l with
  | Init.Datatypes.nil => Init.Datatypes.nil
  | (Init.Datatypes.cons x r) => (infix_plpl (reverse r)
      (Init.Datatypes.cons x Init.Datatypes.nil))
  end.

Axiom rev_aux : forall {a:Type} {a_WT:WhyType a}, forall (x:a) (l:(list a)),
  ((reverse (infix_plpl l
  (Init.Datatypes.cons x Init.Datatypes.nil))) = (Init.Datatypes.cons x (reverse l))).
...
(* Why3 assumption *)
Fixpoint lastf (l:(list Z)) {struct l}: Z :=
  match l with
  | Init.Datatypes.nil => 0%Z
  | (Init.Datatypes.cons x Init.Datatypes.nil) => x
  | (Init.Datatypes.cons x t) => (lastf t)
  end.

Axiom lastf_aux : forall (x:Z) (t:(list Z)), (~ (t = Init.Datatypes.nil)) ->
  ((lastf (Init.Datatypes.cons x t)) = (lastf t)).

Axiom lastf1 : forall (x:Z) (l:(list Z)), (last x l) -> (x = (lastf l)).

Axiom check_list : forall {a:Type} {a_WT:WhyType a}, forall (l:(list a)),
  (l = Init.Datatypes.nil) \/ ~ (l = Init.Datatypes.nil).

(* Why3 goal *)
Theorem lastf2 : forall (l:(list Z)), (~ (l = Init.Datatypes.nil)) -> (last
  (lastf l) l).
(* Why3 intros l h1. *)
Proof.
intros l h1.
